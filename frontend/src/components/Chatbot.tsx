    ...
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 p-4 rounded-full shadow-2xl transition-all z-50 flex items-center justify-center
          ${isOpen ? 'bg-gray-900 text-white rotate-90 scale-90' : 'bg-[var(--color-primary)] text-white hover:scale-105 hover:shadow-[var(--color-primary)]/40'}`}
      >
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 w-[350px] md:w-[400px] h-[500px] max-h-[70vh] bg-white rounded-3xl shadow-2xl border border-gray-200 z-50 flex flex-col overflow-hidden animate-fade-in scale-in origin-bottom-right">
          {/* Header */}
          <div className="bg-gradient-to-r from-[var(--color-primary)] to-yellow-500 p-4 text-white flex items-center gap-3">
            <div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-sm">
              <MessageSquare size={20} className="text-white" />
            </div>
            <div>
              <h3 className="font-bold text-sm">AI Project Assistant</h3>
              <p className="text-[10px] text-white/80 uppercase tracking-widest font-black">ChatGPT 5.5 Powered</p>
            </div>
          </div>
          ...
          <form onSubmit={sendMessage} className="p-3 bg-white border-t border-gray-100">
            <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-2xl p-1 pr-2 focus-within:border-[var(--color-primary)] focus-within:ring-2 focus-within:ring-[var(--color-primary)]/20 transition-all">
              <input
                type="text"
                value={input}
                onChange={e => setInput(e.target.value)}
                placeholder="Ask about budgets, standings..."
                className="flex-1 bg-transparent border-none focus:outline-none px-3 py-2 text-sm text-gray-700"
              />
              <button
                type="submit"
                disabled={!input.trim() || isLoading}
                className="w-8 h-8 flex items-center justify-center bg-[var(--color-primary)] text-white rounded-xl shadow-md disabled:opacity-50 disabled:cursor-not-allowed hover:scale-105 active:scale-95 transition-all"
              >
