        ...
        <main className="flex-1 overflow-y-auto p-8 custom-scrollbar">
            {activeTab === 'timeline' ? (
                <div className="space-y-10">
                    ...
                    {/* Timeline Grid */}
                    <div className="relative pl-12 space-y-8 before:absolute before:left-[19px] before:top-2 before:bottom-2 before:w-[2px] before:bg-gray-100 before:rounded-full">
                        {activities.map((act) => (
                            <div key={act.id} className="relative group/act">
                                {/* Dot Icon */}
                                <div className={`absolute -left-[40px] top-0 w-6 h-6 rounded-lg flex items-center justify-center border-4 border-white shadow-sm transition-all group-hover/act:scale-110 z-10 ${isFuture(act.activity_date) ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-500'}`}>
